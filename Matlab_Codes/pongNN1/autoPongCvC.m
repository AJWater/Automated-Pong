...
BALL_ACCELERATION = 0.03; %how much ball accelerates each bounce.
PADDLE_SPEED = 1.3;
...
MESSAGE_INTRO = [...
  ...
  ' use (w) and (s)     use arrow keys' 10 10 ...
  ];
...
%called from main loop on every frame
  function movePaddles(paddle1V,paddle2V)
...
Theta5 = [];
Theta6 = [];
load('pongThetas1.mat');
load('pongThetas2.mat');
while ~quitGame  
  moveBall;
  X1 = [(ballX-75)/75,(ballY-50)/50,(ballSpeed-5.21)/2.81,...
      ballVector(1),ballVector(2),(p1Center(2)-50)/50];
  X2 = [-(ballX-75)/75,(ballY-50)/50,(ballSpeed-5.21)/2.81,...
      -ballVector(1),ballVector(2),(p2Center(2)-50)/50];
  
  %X = [ballX,ballY,ballSpeed,ballVector(1),ballVector(2),p2Center(2)];
  
  paddle1V = predict(Theta1, Theta2, Theta3, X1) -2;
  paddle2V = predict(Theta1, Theta2, Theta3, X2) -2;
  data_X = [data_X;X1];
  %data_X(:,3)
  movePaddles(paddle1V,paddle2V); 
  refreshPlot;    
  checkGoal;
end
close(fig);
